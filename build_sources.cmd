...
dir "%bDir%" ||fail 7
    ...
    popd
    pushd "%bDir%\src"
    call configure.bat -release -opensource -confirm-license -prefix "%bDir%\bin\%qt_version%\msvc2019_64" -platform win32-msvc -openssl-linked -nomake tests -nomake examples -skip qtdoc -skip qtwebengine
    jom
...
dir "%bDir%" ||fail 7
    ...
    popd
    pushd "%bDir%\src"
    call configure.bat -release -opensource -confirm-license -prefix "%bDir%\bin\%qt_version%\msvc2019" -platform win32-msvc -openssl-linked -nomake tests -nomake examples -skip qtdoc -skip qtwebengine
    jom
    if %ERRORLEVEL% == 0 jom install
